...
class MecanumController
{
    ...
    public:
        ...
        void move_forward(double speed, double offsetl, double offsetr)
        {
            ...
            ledcWrite(FL_CHANNEL, pwmValue + offsetl);
            ledcWrite(FR_CHANNEL, pwmValue + offsetl);
            ledcWrite(BL_CHANNEL, pwmValue + offsetr);
            ledcWrite(BR_CHANNEL, pwmValue + offsetr);
        }

        void move_backward(double speed, double offsetl, double offsetr)
        {
            // Convert speed to PWM value (assuming speed is given in percentage)
            int pwmValue = map(speed, 0, 100, 0, 4095);
        ...
        void move_forward_pid(float speed, float offsetl, float offsetr){
            // Convert speed to PWM value (assuming speed is given in percentage)
            int pwmValue = map(speed, 0, 100, 0, 4095);
